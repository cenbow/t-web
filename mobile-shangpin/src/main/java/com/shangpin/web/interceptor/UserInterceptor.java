    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (StringUtils.isEmpty(user)) {
            ...
            if (!StringUtils.isEmpty(code)) {
                if (ClientUtil.CheckMircro(useragent) && !processURL.contains(LOGIN)) {
            ...
            if (ClientUtil.CheckOrigin(origin) || ClientUtil.CheckApp(useragent)) {// 判断用户来源
                ...
                boolean flag = SessionUtil.loginFromApp(token, userId, session,userService);
...
//    }
    ...
    private String wxLoginURL(HttpServletRequest request, String returnURL) throws UnsupportedEncodingException {
        return this.getLoginUrl(WXLOGIN,returnURL,request);
    }
    ...
    private String getLoginUrl(String urlLogin,String returnURL ,HttpServletRequest request) throws UnsupportedEncodingException {
        StringBuilder url = new StringBuilder();
        String ctx = request.getContextPath();
        if (!StringUtils.isEmpty(ctx)) {
            url.append(ctx);
        }
        url.append(urlLogin).append("?");
