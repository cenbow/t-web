...
public class ReturnGoodsController extends BaseController {
	...
	public String logisticsInfo(String userId, String orderNo, String orderDetailNo, String productNo, String skuNo, String returnId, Model model, HttpServletRequest request){
		...
		//加载物流公司信息
		List<LogisticsShow> lInfos=bizReturnGoodsService.findListLogisticsDatail();
		JSONArray json = new JSONArray();  
		json.addAll(lInfos);
		model.addAttribute("logisticsInfo", json);
	...
	@ResponseBody
	public ResultBase logisticsSubmit(String applyNo, String logisticsNo, String logisticsCompany,String companyCode, HttpServletRequest request){
		String userId = (String)request.getSession().getAttribute(Constants.SESSION_USERID);
		ResultBase resultBase = bizReturnGoodsService.logisticsSubmitObj(userId, applyNo, logisticsNo, logisticsCompany,companyCode);
		return resultBase;
	}
