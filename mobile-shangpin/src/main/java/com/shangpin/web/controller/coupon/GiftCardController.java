...
public class GiftCardController extends BaseController {
	...
	/** 礼品卡记录列表 */
	private static final String GIFTCARD_NEW_RECORDLIST = "giftCard/2917/record_list";
	/** 查询充值密钥成功 */
	...
	/** 礼品卡set手机 */
	private static final String GIFTCARD_STEP_PAY_SET_TEL = "giftCard/2917/step_pay_phone";
	/** 礼品卡set支付密码 */
	private static final String GIFTCARD_STEP_PAY_SET_PASSWD = "giftCard/2917/set_pay_password";
	/** 礼品卡set支付密码 */
	private static final String GIFTCARD_STEP_PAY_PAGE_PASSWD = "giftCard/2917/step_pay_passwd";
	/** 电子卡充值成功 */
	...
	// 区别新版本2.17
	private static final String ENTITY_KEY_VAR = "102";
	// 卡券绑定礼品卡
	private static final String ENTITY_KEY_CARD = "103";
	@Autowired
	...
	public String giftCardStepTel(String oneKeyToken, ModelMap model) {
		...
		if(org.apache.commons.lang3.StringUtils.isNotBlank(oneKeyToken)&&ENTITY_KEY_VAR.equals(oneKeyToken)){
			return GIFTCARD_STEP_PAY_SET_TEL;
		}
	...
	public String bindTel(String phone, String verifyCode, String oneKeyToken,
		...
		// 判断手机号码已经使用过
		User obj = userService.findUserByUserName(phone);
		if (obj != null) {
		...
		if (bindObj != null && bindObj.isSuccess()) {
			...
			// 去第二步设置支付密码
			return giftCardStepPayPasswd(oneKeyToken, model,null);
	...
	public String giftCardStepPayPasswd(String oneKeyToken, ModelMap model,HttpServletRequest request) {
	    logger.info("skip to steppass==>oneKeyToken:"+oneKeyToken);
		model.addAttribute("oneKeyToken", oneKeyToken);
		if(!ENTITY_KEY_VAR.isEmpty()&&ENTITY_KEY_VAR.equals(oneKeyToken)){
			return GIFTCARD_STEP_PAY_PAGE_PASSWD;
		}
	 ...
	 * @author zghw
	 */
	@RequestMapping("giftCard/setPasswd")
	public String setPasswd(ModelMap model,HttpServletRequest request) {
		User user = getSessionUser(request);
		if(StringUtils.isEmpty(user.getMobile())){
			return null;
		}
		model.addAttribute("phone", user.getMobile().substring(0, 3) + "****" +user.getMobile().substring(7, 11) );
		return GIFTCARD_STEP_PAY_SET_PASSWD;
	}
		...
		if (obj != null && obj.isSuccess()) {
			...
			if (org.apache.commons.lang3.StringUtils.isNotBlank(oneKeyToken)
				...
				return "redirect:/giftCard/entityRecharge?type=101";
					...
					&& ENTITY_KEY_CARD.equals(oneKeyToken.trim())){
	...
	/**
	 * 2.17版本添加新页面 礼品卡绑定支付密码
	 * 
	 * @author wh
	 */
	@RequestMapping(value = "giftCard/cardPayPasswd", method = RequestMethod.POST)
	public String bindPasswd(String passwd,String phone,String mobiCode,
			HttpServletRequest request, ModelMap model) throws Exception {
		String userId = getUserId(request);
        ResultBase result = userService.beVerifyPhoneCode(phone, phone, mobiCode);
        if (result == null || Constants.FAILE.equals(result.getCode())) {
        	model.put("msg","短信验证码错误");
        }else{
        	ResultBase obj = giftCardService.beSetGiftCardPassword(userId, passwd);
        	if (obj != null && obj.getCode().equals("0")) {
        		logger.info("Thread.sleep====start");
        		Thread.sleep(5000);
        		return recharge(request, model);
        	}else if (obj != null){
	...
	public String giftCardList(
			...
			String callbackUrl) {
		String userId = getUserId(request);
		String step = statusToStep(ENTITY_KEY_VAR, userId, model);
		if (step != null) {
			// 转到用户绑定礼品卡信息
			return step;
		}
		...
		return GIFTCARD_NEW_RECORDLIST;
	}
	...
	public String buy( String productId, String amount,	String type, HttpServletRequest request, Model model) {
		String userId = getUserId(request);

		ResultObjOne<GiftCardBuy> giftCardBuys = giftCardService.doGiftCardBuy(	userId, "", productId, amount, "", type, "0");
		...
		if (obj != null && obj.isSuccess()) {
			...
			if (!StringUtils.isEmpty(status)) {
				if ("000".equals(status)) {
					...
					return giftCardStepPayPasswd(type, model,null);
	...
	/*	User user = userService.findUserByUserId(userId);
		...
		if(!user.isBindPhoneTrue()){
			logger.info("statusToStep phone:type--" + type);
			return giftCardStepTel(type, model);
		}
		String bindGiftPassword = user.getBindGiftPassword();
		if(!"1".equals(bindGiftPassword)){
			return giftCardStepPayPasswd(type, model, null);
		}
		return null;*/
	}
    ...
    public String sendVerifyCodeToFriend(HttpServletRequest request){
        ...
        // 发送手机号码
		String data = userService.fromSendVerifyCode(phoneNum, phoneNum, msgTemplate);
		logger.info("短信obj：{}",data);
		if(data!= null){
    ...
    public String oneKeyLogin(HttpServletRequest request,ModelMap model){
        ...
        if (quickUser != null) {
            if ("1".equals(quickUser.getIsNewUser())) {
                ...
                userService.fromSendVerifyCode(sendPhoneNum, sendPhoneNum, msgTemplate);
            }
    ...
    public String oneKeyRecharge(String backFlag,HttpServletRequest request,ModelMap model){
		...
		//您赠送给手机号码xxxx的礼品卡已经被充值。
		 ...
		 if(user2!=null&&org.apache.commons.lang3.StringUtils.isNotBlank(user2.getMobile())){
			 logger.info("user2.getMobile()"+user2.getMobile());
			 String msg = userService.fromSendVerifyCode(user2.getMobile(), user2.getMobile(), msgTemplate);
			 logger.info("礼品卡充值成功，给拥有人发送短息通知结果：{}",msg);
		 }		// 转向充值成功页面
		return GIFTCARD_ELECTRONIC_RECHARGE_SUCCESS;
	}
