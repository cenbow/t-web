			...
			success : function(data,textStatus, XMLHttpRequest) {
				...
				if ("0" == data.code) {//成功
					$("#collectId").val(data.collectId);
					   ...
					   $(".collection_btn").text("已加入愿望清单");
					   $(".collection_btn").addClass("cur");
			            return jShare('成功已加入愿望清单',"",""),
	 ...
	 $.ajax({
			...
			success : function(data,textStatus, XMLHttpRequest) {
				...
				if ("0" == data.code) {//成功
					...
					$(".collection_btn").text("加入愿望清单")
					$(".collection_btn").removeClass("cur");
		            return jShare('已取消成功',"",""),
		            $("#J_userName").addClass("error"),
		            !1;
				} else if ("2" == data.code) {//未登录
