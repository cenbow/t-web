...
$(function(){
    ...
    $('body').delegate('#statusItem li',CLICK,function(e){
        $(this).addClass("cur").siblings().removeClass("cur");
    });
...
function filterboxSelect(){
	...
	if(channnelType!=null){
		$("#statusItem li").each(function(){
			var selectFlag ="";
			var selValue = $(this).attr("id");
			if(selValue != null){
				// 判断头部的选中状态
				if(channnelType==''&&selValue==''){
					selectFlag = "1";
				}else if(selValue == channnelType){  
					selectFlag = "1";
				}else{
					selectFlag ="";
				}
	...
	if(num < htmlUrlLength){
		...
		if ($(window).scrollTop() + $(window).height() >= $(document).height()-50){
			...
			if(ready==1){
				...
				// c_p($("<a ctag='{\"id\":\"SJ6\",\"sid\":\"活动ID\",\"page\":\"目标页码\"}'></a>"));
				  $.ajax({
					  ...
					  success : function(data) {
						...
						if(data.searchResult.productList != null && data.searchResult.productList.length > 0){
							$.each(data.searchResult.productList,function(index,item){
								...
								var $divsymbol = $("<div class='symbol'></div>")
								...
								$a.attr({"href":hrefV});
								$img = $("<img ctag={'id':'BL9','bid':'"+brandNo+"','spu':'"+item.productId+"'} />");
								$img.attr({"src":pic});
								$a.append($img);
								$div.append($a);
	  	...
	  	success : function(data) {
			...
			if(len > 0){
				...
				$.each(data.searchResult.sizeList,function(index,item){
					$li = $("<li ctag={'id':'BL10','bid':'" + branNo + "','size':'"+ item.id  +"'}  ></li>");
			...
			if(len2 > 0){
				...
				$.each(data.searchResult.colorList,function(index1,item2){
					$li2 = $("<li></li>");
					$i= $("<i></i>");
					$img = $("<img ctag={'id':'BL7','bid':'" + branNo + "','color':'"+ item.id  +"'} />");
					$img.attr({"src":path+"/styles/shangpin/images/201502brand/color_cur.png"});
					$i.append($img);
					$a = $("<a ctag={'id':'BL7','bid':'" + branNo + "','color':'"+ item.id  +"'} ></a>");
...
//					$a.addClass("all-color");
					if(item2.rgb == "#ffffff" || item2.rgb == "#FFFFFF"){
						...
						$a.append("<img src='../../styles/shangpin/images/201502brand/other_color.jpg' ctag={'id':'BL7','bid':'" + branNo + "','color':'"+ item.id  +"'}  />"); 
					}else{
						$a.attr({"style":"background: "+item2.rgb});      
					}
					
					$span=$("<span ctag={'id':'BL7','bid':'" + branNo + "','color':'"+ item.id  +"'}  ></span>")
