...
function filterboxSelect(){
	...
	var channnelType = $("#channnelType").val();
	if(channnelType==""){
		channnelType="all";
	}
		...
		success : function(data){
			...
			if(brands != null && brands.length > 0){
				...
				$.each(brands,function(index,item){
					$li_html = $("<li id='" + item.id + "'  ctag={'id':'CC2','cid':'"+no+"','bid':'"+item.id+"'}  >" + item.nameEN + "</li>");
			...
			if(sizes != null && sizes.length > 0){
				...
				$.each(sizes,function(index,item){
					...
					$li_html = $("<li id='" + sizeId + "' ctag={'id':'CC7','cid':'"+no+"','size':'"+item.id+"'} >" + item.id + "</li>");
			...
			if(colors != null && colors.length > 0){
				...
				$.each(colors,function(index,item){
					...
					$li_html.attr("onclick", va);
					$i_html = $("<i><img src='" + path + "/styles/shangpin/images/201502brand/color_cur.png' ctag={'id':'CC3','cid':'" + no + "','color':'"+ item.id  +"'}  /></i>");
					if(item.rgb == "#ffffff" || item.rgb == "#FFFFFF"){
						$a_html = $("<a href='javascript:;' class='white-color' style='background:"+ item.rgb +"' ctag={'id':'CC3','cid':'" + no + "','color':'"+ item.id  +"'} ></a>");
					}else if(item.rgb == null || item.rgb == ''){
						$a_html = $("<a href='javascript:;' class='all-color'><img src='../../styles/shangpin/images/201502brand/other_color.jpg'ctag={'id':'CC3','cid':'" + no + "','color':'"+ item.id  +"'}  /></a>");
					}else{
						$a_html = $("<a href='javascript:;' class='black-color' style='background:"+ item.rgb +"' ctag={'id':'CC3','cid':'" + no + "','color':'"+ item.id  +"'} ></a>");
					}
					$span_html = $("<span id='"+ item.id  +"' ctag={'id':'CC3','cid':'" + no + "','color':'"+ item.id  +"'} >" + item.name + "</span>");
...
//选择销售状态
function statusSale(value){
	$("#channnelType").val(value);
}
						  ...
						  if(data.searchResult.productList != null && data.searchResult.productList.length > 0){
							  $.each(data.searchResult.productList,function(index,item){
								  var pic = item.pic.substring(0,item.pic.indexOf('-')) + "-320-426.jpg";
									...
									$a.attr({"href":hrefV});
									$img = $("<img ctag={'id':'CC6','cid':'"+categoryNo+"','spu':'"+item.productId+"'} />");
									$img.attr({"src":pic});
									$a.append($img);
									$div.append($a);
