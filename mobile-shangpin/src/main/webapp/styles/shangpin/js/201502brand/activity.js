...
if(isTopicId=="true"){
	$("#remind_Layer").show();
	$(".remind_info_button").click(function(){
		$("#remind_Layer").hide();	
	});
}
				...
				$.ajax({
					...
					success: function(data){
						...
						if(data==undefined || data=="" || data == "0" ){
							window.location= $("#toLoginUrl").val();
						}
				}
		});
						  ...
						  success : function(data) {
							...
							if( data.searchResult.productList!=null&&data.searchResult.productList.length > 0){
								$.each(data.searchResult.productList,function(index,item){
									...
									$a.attr({"href":hrefV});
									$img = $("<img  ctag={'id':'SJ1','sid':'"+topicId+"','spu':'"+item.productId+"'} />");
									$img.attr({"src":pic});
									$a.append($img);
									$div.append($a);
		  	...
		  	success : function(data) {
				...
				if(len > 0){
					...
					$.each(data.searchResult.sizeList,function(index,item){
						$li = $("<li ctag={'id':'SJ8','sid':'" + topicId + "','size':'"+ item.id  +"'} ></li>");
				...
				if(len2 > 0){
					...
					$.each(data.searchResult.colorList,function(index1,item2){
						$li2 = $("<li></li>");
						$i= $("<i></i>");
						$img = $("<img ctag={'id':'SJ5','sid':'" + topicId + "','color':'"+ item2.id  +"'} />");
						$img.attr({"src":path+"/styles/shangpin/images/201502brand/color_cur.png"});
						$i.append($img);
						$a = $("<a ctag={'id':'SJ5','sid':'" + topicId + "','color':'"+ item2.id  +"'} ></a>");
						...
						if(item2.rgb == "#ffffff" || item2.rgb == "#FFFFFF"){
							...
							$a.append("<img src='../../styles/shangpin/images/201502brand/other_color.jpg' ctag={'id':'SJ5','sid':'" + topicId + "','color':'"+ item2.id  +"'} />"); 
						}else{
							$a.attr({"style":"background: "+item2.rgb});      
						}
				...
				if(len3 > 0){
					...
					$.each(data.searchResult.brandList,function(index3,item3){
						$li = $("<li ctag={'id':'SJ2','sid':'"+topicId+"','bid':'"+item3.id+"'} ></li>");
