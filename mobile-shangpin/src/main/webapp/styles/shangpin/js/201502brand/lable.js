						  ...
						  success : function(data) {
							...
							if( data.searchResult.productList!=null&&data.searchResult.productList.length > 0){
								$.each(data.searchResult.productList,function(index,item){
									...
									var $divsym = $("<div class='symbol'></div>")
									$a = $("<a></a>");
									$.each(item.flagList,function(iv,flag){
										$divsym.append("<i>"+flag+"</i>");
									}) 
									$divsym.append($a);
									...
									$a.attr({"href":hrefV});
									$img = $("<img ctag={'id':'LC6','cid':'"+tagId+"','spu':'"+item.productId+"'} />");
									$img.attr({"src":pic});
									$a.append($img);
									$div.append($a);
		  	...
		  	success : function(data) {
				...
				if(len > 0){
					...
					$.each(data.searchResult.sizeList,function(index,item){
						$li = $("<li ctag={'id':'LC7','cid':'" + tagId + "','size':'"+ item.id  +"'}  ></li>");
				...
				if(len2 > 0){
					...
					$.each(data.searchResult.colorList,function(index1,item2){
						$li2 = $("<li></li>");
						$i= $("<i></i>");
						$img = $("<img ctag={'id':'LC3','cid':'" + tagId + "','color':'"+ item2.id  +"'} />");
						$img.attr({"src":path+"/styles/shangpin/images/201502brand/color_cur.png"});
						$i.append($img);
						$a = $("<a ctag={'id':'LC3','cid':'" + tagId + "','color':'"+ item2.id  +"'} ></a>");
						...
						if(item2.rgb == "#ffffff" || item2.rgb == "#FFFFFF"){
							...
							$a.append("<img src='../../styles/shangpin/images/201502brand/other_color.jpg' ctag={'id':'LC3','cid':'" + tagId + "','color':'"+ item2.id  +"'}  />"); 
						}else{
							$a.attr({"style":"background: "+item2.rgb});      
						}
						
						$span=$("<span ctag={'id':'LC3','cid':'" + tagId + "','color':'"+ item2.id  +"'} ></span>")
				...
				if(len3 > 0){
					...
					$.each(data.searchResult.brandList,function(index3,item3){
						$li = $("<li ctag={'id':'LC2','cid':'" + tagId + "','bid':'"+ item3.id  +"'}></li>");
