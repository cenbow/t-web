		...
		if(data.code == "0"){
			...
			if(pat_giftcard == "1"){
				...
				if(mainPay == "20" && subPay == "37"){
					...
					window.location.href = path + "/order/pay/ALIWAP?orderId=" + data.orderInfo.orderid;
				}else if(mainPay == "19" && subPay == "49"){
					/*$("#submit_order").text("正在跳转银联支付页面.......");
					window.location.href = path + "/order/pay/UNWAP?orderId=" + data.orderInfo.orderid;
					setTimeout(function(){*/
						window.location.href = path + "/order/pay/union_pay/install?orderId=" + data.orderInfo.orderid;
					//},2000);
				}else if(mainPay == "2" && subPay == "41"){//货到付款
					window.location.href = path + "/order/pay/hdpay/success?orderId=" + data.orderInfo.orderid;
				}else if(mainPay == "27" && subPay == "58"){
					$("#submit_order").text("正在跳转微信支付页面.......");
					window.location.href = path + "/order/pay/WEIXINPUB?orderId=" + data.orderInfo.orderid;
				}else if(mainPay == "27" && subPay == "117"){
					$(".payment_btn").text("正在跳转微信支付页面.......");
					window.location.href = path + "/order/pay/WEIXINWAP?orderId=" + data.orderInfo.orderid;
				}
