	...
	if(ready==1){
		...
		if(data.recProductItem){
			...
			$.each(data.recProductItem.list,function(index,item){
				...
				$a.attr({"href":hrefV});
				$img = $("<img ctag={'id':'M6','spu':'"+item.productId+"'} />");
				...
				$div1.append($divsymbol).append($h5).append($p);
				...
				$strong=$("<strong></strong>").addClass("refer-price").attr({"style":"color:"+productColor.color});
				$strong.text(productColor.priceDesc);
				$span.append($strong);
