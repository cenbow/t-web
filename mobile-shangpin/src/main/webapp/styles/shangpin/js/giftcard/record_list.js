...
$(function(){
	...
	function loadOrder(recordType,pageNo){
		...
		$.ajax({
	        url: path+'/giftCard/list',
	        type:'get',
			data:{"recordType":recordType,"pageNo":pageNo,"callbackUrl":params.callBackUrl},
			dataType:'html',
		    async:true,
			success:function(data){
				params.content.find(".setup").remove();
				params.content.find(".purchase-records-list").remove();
				params.content.find(".line").remove();
				params.content.find("#geft_cell").remove();
