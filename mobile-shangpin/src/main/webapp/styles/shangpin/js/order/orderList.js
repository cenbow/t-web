		    ...
		    if(entityJson.code == 0){
	        	...
	        	$( entityJson.result.list).each(function(n,v){
	        		...
	        		$(v.orderList).each(function(n1,v1){
		        		...
		        		$a2=$("<a></a>").attr({"href":"#"}).html("<strong style='color:#888'>"+desc+"：</strong>"+v1.orderId +"<time style='color:#0700c6' >"+v1.statusName +"</time>");
		        		$p2.append($a2);
		        		$fieldset.append($p2);
		        		var flag=0;
		        		$(v1.detail).each(function(n2,v2){
			        		...
			        		if(v.canPay=="1"){
			        			$a4=$("<a></a>")
			        			flag=1;
			        			herfv=path+"/order/pay_normal?orderId="+v1.orderId;
			        			if(v1.postArea=='2'){
...
$(function(){
	$("nav.alOrder_listMenu ul li").click(function(){
        var status = $(this).children("a").attr("status");
        var path = getRootPath();
		window.location.href=path+"/order/list?statusType="+status;
	})
})