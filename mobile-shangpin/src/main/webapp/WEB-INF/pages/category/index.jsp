...
<rapid:override name="page_title">
	品类列表
	<%-- <a href="${ctx }/brand/list"><strong>11</strong></a> --%>
</rapid:override>
					...
					<c:forEach var="navList" items="${navigation.categoryList}"
						...
						<c:if test="${navList!= null && fn:length(navList.id) > 0 && navList.id ne '0'}">
							<li <c:if test="${navList.id == categoryId}">class="cur"</c:if>>
							  ...
							  <c:choose>
							    <c:when test="${navList.id eq '10000'}">
							     <a href="" ctag='{"id":"N1","mid":"${navList.id}","mname":"${navList.name}","midx:":"${status.count}"}'><strong ctag='{"id":"N1","mid":"${navList.id}","mname":"${navList.name}","midx:":"${status.count}"}'>${navList.name}</strong></a>
							    </c:when>
							   <c:otherwise>
							     <a href="" ctag='{"id":"N1","mid":"${navList.id}","mname":"${navList.name}","midx:":"${status.count}"}'><strong ctag='{"id":"N1","mid":"${navList.id}","mname":"${navList.name}","midx:":"${status.count}"}'>${navList.name}</strong></a>
							   </c:otherwise>
