...
<rapid:override name="content">
    ...
    <form>
    <c:if test="${ statusType ne '3' }">
        <p class="attention">提示：${result.noticeInfo }</p>
    </c:if>
   <%-- <div class="order-prompt">&lt;%&ndash;<i><img src="${cdn:js(pageContext.request)}/styles/shangpin/images/order/prompt_img.png" width="26" height="26"></i>&ndash;%&gt;<p class="attention">提示：${result.noticeInfo }</p></div>--%>

    <nav class="alOrder_listMenu">
        <ul class="tabs_menu">
            <li ${ statusType== "1" ? "class='cur'":""} ><a href="javascript:;" status="1">全部订单</a></li>
            <li ${ statusType== "2" ? "class='cur'":""}><a href="javascript:;" status="2">待支付</a>
                <c:if test="${waitPayCount != 0}">
                    <em>${waitPayCount}</em>
                </c:if>
            </li>
            <li ${ statusType== "4" ? "class='cur'":""}><a href="javascript:;" status="4">待发货</a>
                <c:if test="${ waitDeliverCount != 0}">
                    <em>${waitDeliverCount}</em>
                </c:if>
            </li>
            <li ${ statusType== "3" ? "class='cur'":""}><a href="javascript:;" status="3">待收货</a>
                <c:if test="${waitFetchCount != 0}">
                    <em>${waitFetchCount}</em>
                </c:if>
            </li>
        </ul>
    </nav>
    <c:choose>
        <c:when  test="${result.list != null }">
            <c:forEach items="${result.list}" var="mainOrder"  varStatus="i" >
                ...
                <c:forEach items="${mainOrder.orderList}" var="order" varStatus="status" >
                    ...
                    </strong>${order.orderId }  <time style="color:#0700c6" >${order.statusName }</time></a></p>
                    <!--<h2>您在尚品香港购买的商品</h2>-->
                    ...
                    <c:set var="toPayUrl" value="${ctx}/order/pay/payments?orderId=${mainOrder.mainOrderId }" scope="page"/>
        ...
        <c:otherwise>
            ...
            <div class="good_none">
                <img src="${cdn:pic(pageContext.request)}/styles/shangpin//images/member/good_none.png">
                <p>暂无待收货订单</p>
            </div>
