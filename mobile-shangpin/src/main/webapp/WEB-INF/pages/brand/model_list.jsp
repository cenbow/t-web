  ...
  <c:forEach var="model" items="${modelOne.model}" varStatus="status">
	...
	<c:choose>
		...
		<c:when test="${model.type == '2'}">
			<a href="${ctx}/category/product/list?filters=${model.filters}" ${one_stye }>
		</c:when>
...
<%-- 			<img alt="" src="${cdn:pic(pageContext.request)}/styles/shangpin/images/e.gif" lazy="${fn:substring(model.pic,0,fn:indexOf(model.pic,'-'))}-${model.width}-${model.height}.jpg" width="100%">
 --%>			<img alt="" src="${fn:substring(model.pic,0,fn:indexOf(model.pic,'-'))}-${model.width}-${model.height}.jpg" width="100%"  ctag='{"id":"BL1","location":"${(statu.index)+1 }","type":"${modelOne.type }","id":"${model.refContent}"}'>
				   </a>
		</c:when>
		<c:when test="${modelOne.type=='100' ||modelOne.type=='101'}">
			<img alt="" src="${fn:substring(model.pic,0,fn:indexOf(model.pic,'-'))}-${model.width}-${model.height}.jpg"  ctag='{"id":"BL1","location":"${(statu.index)+1 }","type":"${modelOne.type }","id":"${model.refContent}"}' /></a>
		</c:when>
		<c:otherwise>
			<img src="${fn:substring(model.pic,0,fn:indexOf(model.pic,'-'))}-${model.width}-${model.height}.jpg" ctag='{"id":"BL1","location":"${(statu.index)+1 }","type":"${modelOne.type }","id":"${model.refContent}"}' ></a></li>
		</c:otherwise>
    </c:choose>
</c:forEach>
