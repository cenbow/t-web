				        ...
				        <input type="hidden" id ="sku" name="sku" value="${first.secondProps[0].sku }"/>
				    	 ...
				    	 <c:choose>
							<c:when test="${first.secondProps[0].isPromotion == '1'}">
								...
								<span type="hidden" id ="lowestInfo" name="lowestInfo" isPromotion="${first.secondProps[0].isPromotion}" sku="${first.secondProps[0].sku}" lowestPrice="${first.secondProps[0].taxPromotionPrice}" isExchange="${first.secondProps[0].isExchange }" marketPrice="${first.secondProps[0].taxStandardPrice }"></span>
							</c:when>
							<c:otherwise>
								<%-- <c:choose>
                ...
                <span>
                  <c:choose>
                      <c:when test="${productDetail.basic.collect.isCollected=='1'}">
                          <a href="javascript:;" class="collection_btn cur">已加入愿望清单</a>
                      </c:when>
                      <c:otherwise>
                          <a href="javascript:;" class="collection_btn ">加入愿望清单</a>
                      </c:otherwise>
                  </c:choose>
		      	  <c:choose>
