	...
	<div class="theme-box clr">
		<a href="${ctx }/index/release/list?pageIndex=1&pageSize=20&origin=1">
			<div class="theme-left">
				...
				<img alt="" src="${cdn:pic(pageContext.request)}/styles/shangpin/images/e.gif" lazy="${fn:replace(releasesSPActivity.list[0].pic,'-10-10','-400-186') }" width="100%">
			</div>
		</a>
...
<c:if test="${modelOne != null}">
	<c:forEach var="list" items="${modelOne.list}" varStatus="st">
		<c:choose>
...
<c:if test="${newGoodsTitle != null}">
	...
	<h2 class="title">
		${newGoodsTitle.title }
		<a href="${ctx }/new/goods" class="title-more" ctag='{"id":"M3"}'>更多<i class="icon-caret-right"></i></a>	</h2>
	<div class="swiper-container" id="swiper-container3">
			...
			<c:forEach var="list" items="${newGoodsTitle.list}">
				<div class="swiper-slide">
					<c:choose>
						...
						<c:otherwise>
							<a href="${ctx }/brand/product/list?brandNo=${list.refContent}&postArea=0"  ctag='{"id":"M4","bid":"${list.refContent}"}' >
						</c:otherwise>
					</c:choose>
        <img alt="" src="${cdn:pic(pageContext.request)}/styles/shangpin/images/e.gif" lazy="${fn:replace(list.productList[0].pic,'-10-10','-200-266') }" width="100%" >
			        <span class="new-brand-logo"><img src="${cdn:pic(pageContext.request)}/styles/shangpin/images/e.gif" lazy="${fn:replace(list.pic,'-10-10','-100-46') }"/>
			        <em class="model-num">
