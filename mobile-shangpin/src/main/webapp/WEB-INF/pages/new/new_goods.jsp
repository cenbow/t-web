			...
			<c:forEach var="brandList" items="${newGoods.brandList }">
				<div class="new-arrival-box clr">
					...
					<c:choose>
						...
						<c:otherwise>
							<a href="${ctx }/brand/product/list?brandNo=${brandList.refContent}&postArea=0"  class="all-new" ctag='{"id":"NM1","bid":"${brandList.refContent}"}'>
						</c:otherwise>
					</c:choose>
							全部新品<i class="icon-angle-right"></i>
							</a>
					</h2>
						...
						<c:forEach var="product" items="${brandList.productList }">
							<li>
								<c:choose>
									...
									<c:otherwise>
										<a href="${ctx }/brand/product/list?brandNo=${brandList.refContent}&postArea=0"  class="all-new" ctag='{"id":"NM2","bid":"${brandList.refContent}"}'>
									</c:otherwise>
								</c:choose>
								<%--需求：去掉飞机标
								...
								<c:choose>
									<c:when test="${product.isPromotion eq '1'}">
										<span>&yen; ${product.taxPromotionPrice  }</span>
									</c:when>
									<c:otherwise>
										<span>&yen; ${product.taxStandardPrice  }</span>
									</c:otherwise>
								</c:choose>
										</a>
							</li>
