...
<rapid:override name="custum">
	<link href="${cdn:css(pageContext.request)}/styles/shangpin/css/search/list.css${ver}" rel="stylesheet" />
	...
	<script type="text/javascript" charset="utf-8">
			...
			.using("${cdn:js(pageContext.request)}/styles/shangpin/js/search/filterSearch.js${ver}")
			.excute(function(){
			});
	</script>
</rapid:override >
...
<rapid:override name="content">
	<form id="search_form" action="${ctx}/search/list" method="post">
	...
	<%--	<input type="hidden" id="brandNo" name="brandNo" value="${searchConditions.brandNo}"/>--%>
		<input type="hidden" id="keyword" name="keyword" value="${searchConditions.keyword}"/>
	...
	<div class="tabSlider" id="navLine"> 
		<div class="menu-nav">
				<ul class="tabSlider-hd" id="list_menu">
					<li id="defaultOrder" class="curr"><a ctag='{"id":"S7","kw":"${searchConditions.keyword}","sort":"default"}'>默认</a></li>
					<li id="normalOrder"><a ctag='{"id":"S7","kw":"${searchConditions.keyword}","sort":"newest"}' >新品</a></li>
				<%-- 	<c:if test="${searchResult.abroad=='1'}">
						<li id="abroadOrder"><a>海外购</a></li>
					</c:if> --%>
					<li id="hotOrder"><a ctag='{"id":"S7","kw":"${searchConditions.keyword}","sort":"sales"}' >销量</a></li>
					<li class="price-btn" id="treeOrder"><a ctag='{"id":"S7","kw":"${searchConditions.keyword}","sort":"price"}' >价格</a></li>
				</ul> 
		</div>
		<div class="list-box">
			<c:choose>
			    <c:when test="${fn:length(searchResult.productList) != 0 && searchResult.suggest ne ''}">
					<p class="list-recommend">找不到您要的结果，为您推荐了${searchResult.suggest }以下相关商品~</p>
				</c:when>
	...
	<c:forEach var="product" items="${searchResult.productList}" varStatus="status">
		<div class="list_box">
			...
			<c:otherwise>
                ...
                <img src="${cdn:pic(pageContext.request)}/styles/shangpin/images/e.gif"  lazy="${fn:substring(product.pic,0,fn:indexOf(product.pic,'-'))}-320-426.jpg" style="opacity: 1;" ctag='{"id":"S1","kw":"${searchConditions.keyword }","spu":"${product.productId}"}'/>
            </a>
			<div class="li_text">
				<div class="symbol">
			...
			<h2 class="filter_menu"><a href="#" class="close_Btn" id="filter_close"><img src="${cdn:pic(pageContext.request)}/styles/shangpin/images/201502brand/sx_icon.png" height="20"  /></a><p>筛选</p></h2>
				<div class="filter_content">
								...
								<c:forEach var="category" items="${searchFacet.category}" varStatus="category_stu">
									<c:choose>
										<c:when test="${category_stu.index == 0 }">
											<li class="cur" onclick="selectCategory('${category.id}','${category.name}',this)" id="${category.id}" ctag='{"id":"S3","kw":"${category.name}","cid":"${category.id}"}'>${category.name}</li>
										</c:when>
										<c:otherwise>
											<li onclick="selectCategory('${category.id}','${category.name}',this)" id="${category.id}" ctag='{"id":"S3","kw":"${category.name}","cid":"${category.id}"}'>${category.name}</li>
										</c:otherwise>
									</c:choose>
								</c:forEach>
							</ul>
						...
						<div class="category-box brandList">
							...
							<ul>
								<c:forEach var="brand" items="${searchResult.brandList}">
									<li onclick="selectBrand('${brand.id}')" id="${brand.id}" ctag='{"id":"S2","kw":"${brand.nameEN}","bid":"${brand.id}"}'>${brand.nameEN}</li>
								</c:forEach>
							</ul>
						</div>
					</c:if>
									 ...
									 <li onclick="selectColor('${color.id}','${color.name}')" id="${color.id}">
							          	...
							          	<c:choose>
							          		<c:when test="${color.rgb == '#ffffff' || color.rgb == '#FFFFFF'}">
							          			<a href="javascript:;" class="white-color" style="background: ${color.rgb };" ctag='{"id":"S5","kw":"${color.name}","color":"${color.id}"}'></a>
							          		</c:when>
							          		<c:when test="${color.rgb == null || color.rgb == '' }">
							          			<a href="javascript:;" class="all-color"><img src="${cdn:pic(pageContext.request)}/styles/shangpin/images/201502brand/other_color.jpg" ctag='{"id":"S5","kw":"${color.name}","color":"${color.id}"}'/></a>
							          		</c:when>
							          		<c:otherwise>
							          			<a href="javascript:;" class="black-color" style="background: ${color.rgb };" ctag='{"id":"S5","kw":"${color.name}","color":"${color.id}"}'></a>
							          		</c:otherwise>
							          	</c:choose>
							          	
							            <span id="${color.id}" ctag='{"id":"S5","kw":"${color.name}","color":"${color.id}"}'>${color.name}</span>
							         </li>
								</c:forEach>
					        </ul>
					    </div>
				    ...
				    <div class="category-box" id="priceItem">
				    	<h3>价格区间</h3>
				        <ul>
				          <li onclick="selectPrice('0-500')" id="0-500" ctag='{"id":"S4","kw":"0-500","price":"0-500"}'>0-500</li>
				          <li onclick="selectPrice('500-1000')" id="500-1000" ctag='{"id":"S4","kw":"500-1000","price":"500-1000"}'>500-1000</li>
				          <li onclick="selectPrice('1000-2000')" id="1000-2000" ctag='{"id":"S4","kw":"1000-2000","price":"1000-2000"}'>1000-2000</li>
				          <li onclick="selectPrice('2000-5000')" id="2000-5000" ctag='{"id":"S4","kw":"2000-5000","price":"2000-5000"}'>2000-5000</li>
				          <li onclick="searchKey('price','5000-10000');" id="5000-10000"ctag='{"id":"S4","kw":"5000-10000","price":"5000-10000"}' >5000-10000</li>
				          <li onclick="selectPrice('10000-')" id="10000-" ctag='{"id":"S4","kw":">10000","price":">10000"}'>>10000</li>
				        </ul>
				    </div>
