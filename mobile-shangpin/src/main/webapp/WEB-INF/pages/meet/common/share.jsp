...
<c:choose>
	...
	<input type="hidden" name="_mainImgUrl" id="_mainImgUrl"  value="${ctx }/styles/shangpin/images/share-logo.png"/>
	   <c:if test="${checkAPP}">
		  <div class="shang_share" >
			  <a href="shangpinapp://phone.shangpin/actiongoshare?title=${fn:replace(meet.title,'&', '＆')}&desc=${fn:replace(meet.title,'&', '＆')}&url=http://m.shangpin.com/meet/${meet.id}" style="height:50px;"><img src="${cdn:pic(pageContext.request)}/styles/shangpin/images/b_share.png" width="40" style="width:40px"></a>
		   </div>
		</c:if>
	</c:otherwise>
</c:choose>