...
<%@ include file="/WEB-INF/pages/common/include.inc.jsp"%>
   <!--弹层显示购买信息start-->
    ...
    <div class="alProdInfo">
      ...
      <div class="clr">
       <div class="commodity_info clr">
		...
		<c:when test="${productDetail.basic.firstProps[0].secondProps[0].isPromotion=='0'}">
		  <b class="sales" id="forSalsPrice" >
				<%-- <c:choose>
					...
					<c:otherwise> --%>
						&yen;${productDetail.basic.firstProps[0].secondProps[0].taxStandardPrice }
						<span type="hidden" id ="lowestInfo" name="lowestInfo" sku="${productDetail.basic.firstProps[0].secondProps[0].sku}" isPromotion="${productDetail.basic.firstProps[0].secondProps[0].isPromotion}"  lowestPrice="${productDetail.basic.firstProps[0].secondProps[0].taxStandardPrice}" isExchange="${productDetail.basic.firstProps[0].secondProps[0].isExchange }" marketPrice="${productDetail.basic.firstProps[0].secondProps[0].taxStandardPrice }"></span>			
		...
		<c:otherwise>
		 <b class="sales" id="forSalsPrice" >
			 &yen;${productDetail.basic.firstProps[0].secondProps[0].taxPromotionPrice }
		 </b>
		</c:otherwise>
	</c:choose>
          <p>
    ...
    <c:forEach var="first" items="${productDetail.basic.firstProps}" varStatus="status">
				        ...
				        <input type="hidden" id ="sku" name="sku" value="${first.secondProps[0].sku }"/>
				    	 ...
				    	 <c:choose>
							<c:when test="${first.secondProps[0].isPromotion == '1'}">
									<input type="hidden" id ="salePrice" name="salePrice" value="${first.secondProps[0].taxPromotionPrice }"/>
							</c:when>
							<c:otherwise>
									<%-- <c:choose>
										...
										<c:otherwise> --%>
											<input type="hidden" id ="salePrice" name="salePrice" value="${first.secondProps[0].taxStandardPrice }"/>
										<%-- </c:otherwise>
											</c:choose> --%>
								</c:otherwise>
							</c:choose>
      ...
      <div class="submit" ctag='{"id":"STM2","pid":"${productNo}"}'>
        <a href="javascript:;" class="submit_btn" ctag='{"id":"STM2","pid":"${productNo}"}'>确定</a>
      </div>
