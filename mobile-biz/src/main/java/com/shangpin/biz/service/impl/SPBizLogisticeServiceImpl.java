...
public class SPBizLogisticeServiceImpl extends AbstractBizLogisticeService implements SPBizLogisticeService {
	...
	@Override
	public OrderLogistics getLogisticV3(String orderId, String userId) {


		JSONObject reqeustJson = new JSONObject();
		reqeustJson.element("userId",userId).element("orderId",orderId);
		reqeustJson.element("versionNo", GlobalConstants.ORDER_VERSION);
		reqeustJson.element("requestFrom",GlobalConstants.ORDER_REQUEST_FRMO);
		String json = shangpinService.findOrderMoreLogistic(reqeustJson.toString());
