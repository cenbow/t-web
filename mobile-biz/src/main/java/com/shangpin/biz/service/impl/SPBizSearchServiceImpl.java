...
@Service
public class SPBizSearchServiceImpl  extends AbstractBizSearchService implements SPBizSearchService {
	...
	public SearchResult queryLabels(String pageIndex, String pageSize,
		...
		if (!StringUtils.isEmpty(xmlStr)) {
			SearchResult searchResult=SearchUtil.converXmlToObj(xmlStr, "2",searchParam.getCategoryId(),null, null);
			return searchResult;
		}
		return null;
	}
	...
	public SearchKeyword getSearchKeyword() throws Exception {
		String json= commonService.querySearchKeyword();
		 if(!StringUtils.isEmpty(json)){
	           CommonObj obj = JSONUtils.toGenericsCollection(json, CommonObj.class, SearchKeyword.class);
	           return (SearchKeyword) obj.getContent();
	        }
	      return null;
	}
	...
	public ResultSuggestion suggestion(String keywords) {
		return getSearchResult(keywords.toLowerCase(), "", "", "UTF-8");
	}

	@Override
	public SearchResult searchList(SearchConditions search) {
		try {
			
			String xml = searchService.searchProductListByKeyWord(search.getKeyword(), search.getStart(), search.getNum(), search.getTagId(), search.getBrandNo(), search.getPrice(),
					search.getColor(), search.getSize(), search.getCategoryNo(),search.getOrder(), search.getUserLv(), search.getPostArea());
			if (!StringUtils.isEmpty(xml)) {
				try {
				  return SearchUtil.converXmlToObj(xml, "5", search.getCategoryNo(), null, null);
	...
	public SearchResult queryActivityProductList(String subjectNo, String start, String end, String tagId, String brandId, String price, String colorId, String size,
		...
		try {
			...
			if (!StringUtils.isEmpty(xmlStr)) {
				searchResult = SearchUtil.converXmlToObj(xmlStr, "1", categoyId, activityHead, version);
			}
		} catch (UnsupportedEncodingException e) {
			logger.error("调用base接口返回数据发生错误！");
			e.printStackTrace();
	...
	public SearchResult queryBrandProductList(String navId, String pageIndex, String pageSize, String tagId, String brandId, String price, String colorId, String size,
		...
		SearchResult searchResult = new SearchResult();
		try {
			String xmlStr = searchService.searchBrandShopProductList(navId, pageIndex, pageSize, tagId, brandId, price, colorId, size, categoyId, order, userLv, postArea, imei,channnelType);
			logger.debug("调用base接口返回数据:" + xmlStr);
			if (!StringUtils.isEmpty(xmlStr)) {
				searchResult = SearchUtil.converXmlToObj(xmlStr, "2", categoyId, null, version);
			}
		} catch (UnsupportedEncodingException e) {
			logger.error("调用base接口返回数据发生错误！");
			e.printStackTrace();
	...
	public SearchResultApp queryBrandProductListNew(String navId, String pageIndex, String pageSize, String tagId, String brandId, String price, String colorId, String size,
		...
		try {
			String xmlStr = searchService.searchBrandShopProductList(navId, pageIndex, pageSize, tagId, brandId, price, colorId, size, categoyId, order, userLv, postArea, imei,null);
			logger.debug("调用base接口返回数据:" + xmlStr);
			if (!StringUtils.isEmpty(xmlStr)) {
				searchResultApp = SearchUtil.searchByApp(SearchUtil.converXmlToObj(xmlStr, "2", categoyId, null, version), "2");
			}
		} catch (UnsupportedEncodingException e) {
			logger.error("调用base接口返回数据发生错误！");
			e.printStackTrace();
		...
		try {
			...
			if (!StringUtils.isEmpty(xmlStr)) {
				if (com.shangpin.utils.StringUtil.compareVersion("", "2.9.7", version) == 1) {
					searchResultApp = SearchUtil.searchByApp(SearchUtil.converXmlToObj(xmlStr, "2", categoyId, null, version), "2", originalFilters, dynamicFilters);
					if(CollectionUtils.isEmpty(searchResultApp.getProductList())) {
						//排除价格区间和发货地重新查询筛选条件
						xmlStr = searchService.searchBrandShopProductList(navId, pageIndex, pageSize, tagId, brandId, null, colorId, size, categoyId, order, userLv, null, imei,null);
						searchResultApp = SearchUtil.searchByApp(SearchUtil.converXmlToObj(xmlStr, "2", categoyId, null, version), "2", originalFilters, dynamicFilters);
	...
	public SearchResultApp queryActivityProductListNew(String subjectNo, String start, String end, String tagId, String brandId, String price, String colorId, String size,
		...
		try {
			...
			logger.debug("调用base接口返回数据:" + xmlStr);
			if (!StringUtils.isEmpty(xmlStr)) {
				searchResultApp = SearchUtil.searchByApp(SearchUtil.converXmlToObj(xmlStr, "1", categoyId, activityHead, version), "1");
			}
		} catch (UnsupportedEncodingException e) {
			logger.error("调用base接口返回数据发生错误！");
			e.printStackTrace();
	...
	public SearchResultApp queryActivityProductListNew(String subjectNo, String start, String end, String tagId, String brandId, String price, String colorId, String size,
		...
		try {
			...
			if (!StringUtils.isEmpty(xmlStr)) {
				if (com.shangpin.utils.StringUtil.compareVersion("", "2.9.7", version) == 1) {
					searchResultApp = SearchUtil.searchByApp(SearchUtil.converXmlToObj(xmlStr, "1", categoyId, activityHead, version), "1", originalFilters, dynamicFilters);
					if(CollectionUtils.isEmpty(searchResultApp.getProductList())) {
						//排除价格区间和发货地重新查询过滤条件
	...
	public String queryTagProductListNew(String pageIndex, String pageSize, String userLv, String tagId, String order, String filters, String version) throws Exception {
		...
		if (!StringUtils.isEmpty(xmlStr)) {
			String json = null;
			try {
				searchResultApp = SearchUtil.searchByApp(SearchUtil.converXmlToObj(xmlStr, "2", searchParam.getCategoryId(), null, version), "4");
	...
	public String queryKeyWordProductList(String keywords, String pageIndex, String pageSize, String tagId, String order, String userLv, String filters, String version) throws Exception {
		...
		if (!StringUtils.isEmpty(xmlStr)) {
			String json = null;
			try {
				searchResultApp = SearchUtil.searchByApp(SearchUtil.converXmlToObj(xmlStr, "5", searchParam.getCategoryId(), null, version), "5");
				json = ApiBizData.spliceData(searchResultApp, CodeMsgEnum.CODE_SUCCESS.getInfo(), CodeMsgEnum.MSG_SUCCESS.getInfo());
				return json;
			} catch (UnsupportedEncodingException e) {
				logger.error("调用base接口返回数据发生错误！");
	...
	public String queryKeyWordProductList(String keywords, String pageIndex, String pageSize, String tagId, String order, String userLv, String filters, String originalFilters, String dynamicFilters, String version) throws Exception {
		...
		if (!StringUtils.isEmpty(xmlStr)) {
			...
			try {
				if (com.shangpin.utils.StringUtil.compareVersion("", "2.9.7", version) == 1) {
					searchResultApp = SearchUtil.searchByApp(SearchUtil.converXmlToObj(xmlStr, "5", searchParam.getCategoryId(), null, version), "5", originalFilters, dynamicFilters);
					if(CollectionUtils.isEmpty(searchResultApp.getProductList())){
						//排除价格区间和发货地重新获取过滤条件
	...
	public String queryCategoryProductList(String pageIndex, String pageSize, String userLv, String tagId, String order, String filters, String imei, String version) throws Exception {
		...
		String xmlStr = searchService.searchCategoryProduct(pageIndex, pageSize, searchParam.getBrandId(), searchParam.getPriceId(), searchParam.getColorId(),
				searchParam.getSizeId(), searchParam.getCategoryId(), order, userLv, "", searchParam.getPostArea(), searchParam.getAttributeId(), imei,null);
		if (!StringUtils.isEmpty(xmlStr)) {
			String json = null;
			try {
				searchResultApp = SearchUtil.searchByApp(SearchUtil.converXmlToObj(xmlStr, "5", searchParam.getCategoryId(), null, version), "3");
				json = ApiBizData.spliceData(searchResultApp, CodeMsgEnum.CODE_SUCCESS.getInfo(), CodeMsgEnum.MSG_SUCCESS.getInfo());
				return json;
			} catch (UnsupportedEncodingException e) {
				logger.error("调用base接口返回数据发生错误！");
	...
	public String queryCategoryProductList(String pageIndex, String pageSize, String userLv, String tagId, String order, String filters, String originalFilters, String dynamicFilters, String imei, String version) throws Exception {
		...
		if (!StringUtils.isEmpty(xmlStr)) {
			...
			try {
				if (com.shangpin.utils.StringUtil.compareVersion("", "2.9.7", version) == 1) {
					searchResultApp = SearchUtil.searchByApp(SearchUtil.converXmlToObj(xmlStr, "5", searchParam.getCategoryId(), null, version), "3", originalFilters, dynamicFilters);
					if(CollectionUtils.isEmpty(searchResultApp.getProductList())) {
						//去掉价格区间和发货地重新查询结果集
						xmlStr = searchService.searchCategoryProduct(pageIndex, pageSize, searchParam.getBrandId(), null, searchParam.getColorId(),
								searchParam.getSizeId(), searchParam.getCategoryId(), order, userLv, "", null, searchParam.getAttributeId(), imei,null);
						searchResultApp = SearchUtil.searchByApp(SearchUtil.converXmlToObj(xmlStr, "5", searchParam.getCategoryId(), null, version), "3", originalFilters, dynamicFilters);
						filterUnSelectedAttrs(searchResultApp);
						//移除选择所有商品结果
						searchResultApp.setProductList(new ArrayList<Product>());
...
//				System.out.println("json========="+json);
	...
	@Override
	public String queryLabels(String pageIndex, String pageSize, String userLv, String tagId, String order, String filters, String type, String imei, String version) throws Exception {
		SearchResultApp searchResultApp = new SearchResultApp();
		SearchParam searchParam = SearchParamUtil.parse(filters, "6");
		String area = searchParam.getPostArea();
		area = StringUtils.isEmpty(area) ? "0" : area;
	...
	public String queryLabels(String pageIndex, String pageSize, String userLv, String tagId, String order, String filters, String originalFilters, String dynamicFilters, String type, String imei, String version) throws Exception {
		...
		if (!StringUtils.isEmpty(xmlStr)) {
			if (com.shangpin.utils.StringUtil.compareVersion("", "2.9.7", version) == 1) {
				searchResultApp = SearchUtil.searchByApp(SearchUtil.converXmlToObj(xmlStr, "2", searchParam.getCategoryId(), null, version), "4", originalFilters, dynamicFilters);
				if(CollectionUtils.isEmpty(searchResultApp.getProductList())) {
					//排除价格区间和发货地获取筛选条件
	...
	public RecProductFor searchProductNos(String start, String end, String minPrice,
										  ...
										  String excludeProductNo, String userLv, String version) {
...
/*
	...
	public List<Product> getProductListFromProductNo(List<String> spus, Map<String, String> productAttMap, ActivityHead activityHead, String version) {
		...
		List<Product> productList = SearchUtil.initJsonProductList(searchList, "1", activityHead, version);
		return productList;
	}
