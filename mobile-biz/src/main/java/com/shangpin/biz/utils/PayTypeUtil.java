...
/*            if (!payType.getChannel().equals(channel)) {
      ...
      * @return
      */
    public static List<Payment> getPaymentList(String mainPayId,String isCod,String type, String platform, /*String hasCod,*/ String channel, String version, String pay){
    	...
    	if(paymentType!=null && paymentType.size()>0){
    		...
    		if (com.shangpin.utils.StringUtil.compareVersion("2.9.16", version) == 1) {
				if ("1".equals(pay) && "1".equals(type)) {
					Payment payment = new Payment();
					payment.setId("34");
					payment.setSubpayCode("123");
					payment.setEnable("1");
					payment.setName("Apple Pay");
					if( mainPayId.equals(payment.getId()) ){
	    				payment.setIsSelected("1");
	    			}else{
	    				payment.setIsSelected("0");
	    			}
