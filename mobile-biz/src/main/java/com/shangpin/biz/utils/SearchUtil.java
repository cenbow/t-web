...
public class SearchUtil {
	...
	private static List<SearchBrand> initBrandList(List<Element> element) {
		List<SearchBrand> brandList = new ArrayList<>();
		if (element != null && element.size() > 0) {
			for (int k = 0; k < element.size(); k++) {
				SearchBrand brand = new SearchBrand();
				Element element1 = (Element) element.get(k);
	...
	@SuppressWarnings("rawtypes")
	private static List<Product> initProductList(List element, String fromType, ActivityHead activityHead, String version) {
		...
		if (element != null && element.size() > 0) {
			...
			for (Object obj :element) {
				...
				if (attributes1 != null && attributes1.size() > 0) {
					...
					}else if ("OperatingFlag".equals(fieldName) && StringUtil.isNotEmpty(value)) {
						product.setOperatingFlag(value);
					}else if ("TaxStandardPrice".equals(fieldName) && StringUtil.isNotEmpty(value)) {
						product.setTaxStandardPrice(removeDecimal(value));
					}else if ("TaxPromotionPrice".equals(fieldName) && StringUtil.isNotEmpty(value)) {
						product.setTaxPromotionPrice(removeDecimal(value));
					}else if ("TaxSellPrice".equals(fieldName) && StringUtil.isNotEmpty(value)) {
						product.setTaxSellPrice(removeDecimal(value));
					}else if ("VipPrice".equals(fieldName) && StringUtil.isNotEmpty(value)) {
						product.setVipPrice(removeDecimal(value));
					}
			...
			product = getPromionProduct(product, fromType, activityHead, version);
	 ...
	 * @param version
	 *            m站传null,api和apiv2传App的version
	 * @throws UnsupportedEncodingException
	 * @throws DocumentException
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public static SearchResult converXmlToObj(String data, String fromType, String categoryId, ActivityHead activityHead, String version) throws UnsupportedEncodingException, DocumentException {
		...
		List<SearchCategory> categoryList2 = new ArrayList<>();
		List<SearchCategory> categoryList3 = new ArrayList<>();
		List<Product> productList = new ArrayList<>();
		List<SearchDynamicAttr> searchThemeList = new ArrayList<>();
		String qTime = root.elementText("QTime");
		String total = root.elementText("Total");
				...
				if (elements != null && elements.size() > 0) {
					for (Object obj :elements) {
						...
						if (secondEName.equals("facet") && secondAttrName != null) {
							...
							if (categoryId != null) {
								if (categoryId.length() == 12 || categoryId.contains("A01B03") || categoryId.contains("A02B03")) {
							...
							productList.addAll(initProductList(element.elements(), fromType, activityHead, version));
	...
	private static List<SearchAttribute> searchByAttribute(SearchResult searchResult, String source) {
		...
		if (categoryList != null && categoryList.size() > 0) {
			...
			if (!categoryList.isEmpty() && !categoryList2.isEmpty() && !categoryList3.isEmpty()) {
				...
				for (int i = 0; i < categoryList.size(); i++) {
					...
					if (resultMap.containsKey(category.getParentId())) {
						...
						List<SearchCategory> list = new ArrayList<>();
						list.add(category);
				...
				if (categoryList2.size() > 1) {
					Map<String, SearchCategory> map = new HashMap<>();
					List<SearchAttributeGroup> searchGroups = new ArrayList<>();
			...
			// 2.9.5以前的版本要用
			List<SearchAttributeValue> searchList = new ArrayList<>();
		...
		if (!"2".equals(source) && brandList != null && brandList.size() > 0) {
			...
			for (int i = 0; i < brandList.size(); i++) {
				...
				List<SearchAttributeExt> searchAttributeExtList = new ArrayList<>();
		...
		if (attrList != null && attrList.size() > 0) {
			for (int i = 0; i < attrList.size(); i++) {
				...
				if (searchDynamicAttr != null) {
					...
					List<SearchAttributeGroup> searchAttributeGroups2 = new ArrayList<>();
					...
					List<SearchAttributeValue> SearchAttributeValueList = new ArrayList<>();
	...
	private static List<SearchAttribute> searchByAttribute(SearchResult searchResult, String source, String originalFilters, String dynamicFilters) {
		...
		if (!"2".equals(source) && brandList != null && brandList.size() > 0) {
			...
			if (!((brandList.size() == 1) && 
				...
				SearchAttributeGroup searchAttributeGroup1 = new SearchAttributeGroup();
				List<SearchAttributeGroup> searchAttributeGroups1 = new ArrayList<>();
				searchAttribute = new SearchAttribute();
				searchAttribute.setName("brand");
	...
	private static List<SearchAttribute> searchByfiltrates(SearchResult searchResult) {
		...
		if (!"0".equals(searchResult.getCount())) {
			...
			if (searchResult.getProductList() != null) {
				...
				List<SearchAttributeExt> defaultExtList = new ArrayList<>();
			...
			if (aBroad != null) {
				...
				searchAttributeExtList = new ArrayList<>();
				searchAttributeValue.setExt(searchAttributeExtList);
				searchAttributeValueList.add(searchAttributeValue);
				searchAttributeValue = new SearchAttributeValue();
	 ...
	 * @param searchList 搜索的product集合列表
	 * @param fromType 来源 1:活动;2:品牌;3:品类;4:标签;5:关键字
	 * @param activityHead 当fromType为1时，传值，其他传null
	 */
	public static List<Product> initJsonProductList(List<ProductSreach> searchList, String fromType, ActivityHead activityHead, String version) {
		List<Product> productList = new ArrayList<>();
		if (searchList != null && searchList.size() > 0) {
			for (ProductSreach search : searchList) {
				Product product = new Product();
				product.setProductStatus("0");
				int stock = 0;
				...
				product.setTaxPromotionPrice(StringUtil.isNotEmpty(search.getTaxPromotionPrice())?removeDecimal(search.getTaxPromotionPrice()):"");
				product.setTaxStandardPrice(StringUtil.isNotEmpty(search.getTaxStandardPrice())?removeDecimal(search.getTaxStandardPrice()):"");
				product.setOperatingFlag(StringUtil.isNotEmpty(search.getOperatingFlag())?removeDecimal(search.getOperatingFlag()):"");
				product.setTaxSellPrice(StringUtil.isNotEmpty(search.getTaxSellPrice())?removeDecimal(search.getTaxSellPrice()):"");
				product.setVipPrice(StringUtil.isNotEmpty(search.getVipPrice())?removeDecimal(search.getVipPrice()):"");
				// 这个值以后不用了 写死为1
				product.setIsSupportDiscount(search.getIsSupportDiscount());
					...
					if (!("".equals(search.getPostAreaPic()))) {
						...
						// 因为后面国旗标被去掉了，所以只能在此添加标志
						//product.getFlagList().add("海外直发");
					} else {
						product.setCountryPic("");
					}
					if (stock != 0) {
	...
	public static Product getPromionProduct(Product product,String fromType,ActivityHead activityHead, String version){
		if(product==null){
			return product;
		}
		String nomal_priceDesc;
		String nomal_color;//颜色
		String nomal_compareColor;//颜色
		String nomal_compareDesc;//价格描述文字
		String nomal_compareHasLine;//是否含有删除横线 0:无  1:有

		String vip_color;//颜色
		String vip_priceDesc;//价格
		String vip_compareColor="";//颜色
		String vip_compareDesc;//价格描述文字
		String vip_compareHasLine;//是否含有删除横线 0:无  1:有

		//String priceTitle = null;// 价格title
		//String priceColor =null;// 价格颜色
		//String descColor = null;// 描述颜色
		//String priceDesc = null; // 价格描述
		//String expressLogo = null;// 直发图片

		StringBuffer descBuffer = null;


		String taxPromotionPrice = product.getTaxPromotionPrice();//促销价格
		String taxStandardPrice = product.getTaxStandardPrice(); //尚品价格
		String taxVipPrice = product.getVipPrice(); //vip价格
		String ceil="¥";
		String shangpinPrice_Desc=packagePriceDesc(ceil,taxStandardPrice);
		String promotionPrice_Desc=packagePriceDesc(ceil,taxPromotionPrice);
		String vipPrice_Desc=packagePriceDesc(ceil,taxVipPrice);

		//促销状态
		/**
		 * 非vip: 详情页促销商品展示商品的含税促销价，价格颜色为红色（#c62026）；对比价为含税尚品价，使用灰色（#888888）进行展示，使用横线划掉
		 	vip:  详情页促销商品展示商品的含税促销价，价格颜色为红色（#c62026）；对比价为含税尚品价，使用灰色（#888888）进行展示，使用横线划掉
		 */
		nomal_compareHasLine="1";//是否含有删除横线 0:无  1:有
		vip_compareHasLine="1";//是否含有删除横线 0:无  1:有
		vip_compareColor = nomal_compareColor=PriceColor.NOMAL_COMPARE_PRICE.toString();//灰色

		if ("1".equals(product.getIsPromotion())) {
			vip_color = nomal_color=PriceColor.PROMOTION_PRICE.toString();//红色
			vip_priceDesc = nomal_priceDesc = promotionPrice_Desc;
			vip_compareDesc = nomal_compareDesc = shangpinPrice_Desc;//价格描述文字
		}else {
			nomal_color=PriceColor.SHANGPIN_PRICE.toString();//黑色
			nomal_priceDesc = shangpinPrice_Desc;
			nomal_compareDesc="";

			vip_color = PriceColor.VIP_PRICE.toString();
			vip_priceDesc = vipPrice_Desc;
			vip_compareDesc = "";
		}
		...
		if (StringUtil.compareDate(Constants.ACTIVITY_READY_START, Constants.ACTIVITY_READY_END, DateUtils.dateToStr(new Date())) == 0) {
			if (!"1".equals(product.getIsPromotion()) &&(StringUtil.isNotEmpty(taxPromotionPrice) && (Integer.parseInt(taxPromotionPrice) < Integer.parseInt(taxStandardPrice)))) {
				vip_compareDesc = nomal_compareDesc=packagePriceDesc(Constants.PRICE_ACTIVITY,promotionPrice_Desc);
			}
			...
			if ("1".equals(product.getIsPromotion())) {
				vip_priceDesc = nomal_priceDesc = packagePriceDesc(Constants.PRICE_ACTIVITY,promotionPrice_Desc);
			}
		}else {
			// 活动有预热单独处理
			if ("1".equals(fromType)&&(activityHead != null)&&("1".equals(activityHead.getIsPre()))){
				nomal_compareColor = PriceColor.VIP_PRICE.toString();
				descBuffer = new StringBuffer();
				if ("1".equals(activityHead.getPriceTag())) {
					descBuffer.append(activityHead.getPriceDesc());
					descBuffer.append(":¥" + taxPromotionPrice);
					vip_compareDesc = nomal_compareDesc = descBuffer.toString();
				} else if ("2".equals(activityHead.getPriceTag())) {
					descBuffer.append(activityHead.getPriceDesc());
					vip_compareDesc = nomal_compareDesc = descBuffer.toString();
				}
				nomal_compareHasLine="0";//是否含有删除横线 0:无  1:有
				vip_compareHasLine="0";//是否含有删除横线 0:无  1:有
				vip_compareColor = nomal_compareColor=PriceColor.PROMOTION_PRICE.toString();//红色
			}
		}
		product.setCountryPic("");// 占时去掉国旗标
		product.setPriceColor("");
		product.setDescColor("");
		product.setExpressLogo("");
		product.setPriceTitle("");
		product.setPriceDesc("");
		product.setNomal_ProductPrice(new ProductPrice(nomal_color,nomal_priceDesc,nomal_compareColor,nomal_compareDesc,nomal_compareHasLine));
		product.setVip_ProductPrice(new ProductPrice(vip_color,vip_priceDesc,vip_compareColor,vip_compareDesc,vip_compareHasLine));
		return product;
	}
	/**
	 * 拼装价格
	 ...
	 * @return
	 */
	private static String packagePriceDesc(String config, String price) {
		StringBuffer descBuffer = new StringBuffer();
		if(!StringUtils.isEmpty(config)){
			descBuffer.append(config);
		}
		descBuffer.append(price);
